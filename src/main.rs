...
fn main() {
    ...
    rayon::scope(|s| {
        ...
        s.spawn(|_| {
            Lexer::start_lexing(code, sender);
        });
...
impl<'a> Lexer<'a> {
    pub fn start_lexing(s: &'a str, sender: Sender<Token<'a>>) {
    ...
    fn lex_operation(l: &mut Lexer) -> Option<StateFunction> {
        l.accept_run("+-/*abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789");
